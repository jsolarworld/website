...
- **Staff** (`/admin/staff`, owner only): create accounts (server generates a one-time temporary password), change role, reset sign-in (new temp password + clears authenticator + signs out), remove access (role becomes CUSTOMER). Guard rails in `lib/admin/staff-rules.ts`: no self-changes, never zero owners. Everyone changes their own password on `/admin/security`. Analytics: `@vercel/analytics` and `@vercel/speed-insights` render via `SiteChrome`, so they run on the public site only, never under `/admin`.
- **Leads** (`/admin/leads`): status pipeline for quote leads. Not built yet: orders, service bookings, content/blog editor, delivery zones, settings/quote-settings editor, audit-log viewer, customer accounts.

## Commerce (built)

Cart, checkout and orders live in `lib/orders/` (pure rules + tests in `rules.ts`, `cart.ts`, `checkout-form.ts`, `paystack.ts`; database logic in `service.ts`).

- **Cart** is a cookie of `[productId, qty]` pairs (`jsw_cart`, no prices; not httpOnly so the header `CartLink` can show a count). Everything in it is untrusted: prices, stock and availability are re-read from the DB on every render and at order creation. Only in-stock, non-"available on request" products can be added.
- **Order creation** (`createOrder`): one transaction that validates, reserves stock with a conditional `updateMany ... stock >= qty` decrement (two buyers can't take the last unit), and writes Order + items (price snapshots) + Payment + first event. A person or email may hold at most 3 unpaid orders (stock-hoarding guard). Unpaid orders expire after `orders.settings.holdHours` (default 24, editable in admin settings).
- **Payments:** Paystack (verify-based, no webhooks) or manual bank transfer. `confirmPayment` is idempotent (conditional claim of the Payment row) and also handles a payment arriving after the order was cancelled: it re-reserves stock, or marks the order Paid with a loud "PAID AFTER EXPIRY, contact the customer" event when stock is gone. Cancelling deliberately leaves payments untouched so late money is never ignored. `evaluateVerification` rejects wrong amount, currency or reference. Bank transfers become Paid only when staff tick "money is in the bank account" at `/admin/orders/[id]` (a receipt image is evidence, not proof). Transfer receipts upload through `/api/order/proof` (server-side to Cloudinary, validated), never a public signing endpoint.
- **No webhook safety net:** `reconcileOrder` re-verifies open Paystack payments when the customer views their order, when staff press "Re-check", and inside `expireStaleOrders`, which runs lazily (cart, checkout, admin orders) and daily from Vercel Cron (`vercel.json` -> `/api/cron/expire-orders`, needs `CRON_SECRET`). It skips orders that have a transfer receipt waiting.
- **Customer access:** `/order/[orderNumber]?t=<token>`; the token is an HMAC of the order number with `BETTER_AUTH_SECRET` (`orderToken`), so knowing a number is not enough. `/track-order` swaps order number + phone/email for a link. `/pay/callback` verifies server-side before redirecting; the redirect itself proves nothing.
- **Settings** (`/admin/settings`, owner only): bank account (FCMB 1049984602, J SOLAR WORLD ENERGY, seeded) and hold hours, stored in the `Setting` table via `lib/settings.ts` with code defaults; every change is audit-logged.
- **Delivery:** the price online is for goods only; delivery cost is confirmed with the customer before dispatch (owner: customer pays transport). `deliveryFeeNgn` is 0 for now; delivery zones are not built.
- **Env vars for commerce:** `PAYSTACK_SECRET_KEY`, `PAYSTACK_PUBLIC_KEY`, `PAYSTACK_BASE_URL`, `CRON_SECRET`, `CLOUDINARY_URL`, `BETTER_AUTH_SECRET` (also signs order links: rotating it invalidates existing order links).
